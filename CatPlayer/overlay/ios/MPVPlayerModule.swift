...
class MPVPlayerModule: NSObject {
    ...
    @objc func play(_ uri: String, headers: [String: String]?) {
        ...
        DispatchQueue.main.async { [weak self] in
            ...
            guard let urlObj = URL(string: uri) else {
                self.sendEvent("error", body: ["error": "Invalid URL: \(uri)"])
                return
            }
    ...
    @objc override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey: Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if keyPath == "status" {
            ...
            case .failed:
                let err = item.error?.localizedDescription ?? "Unknown error"
                sendEvent("error", body: ["error": err])
            default: break
            }
        }
    }
    ...
    private func sendEvent(_ name: String, body: [String: Any]) {
        DispatchQueue.main.async {
            // Use RCTEventEmitter or bridge to send events
            // For simplicity, use NotificationCenter
            NotificationCenter.default.post(
                name: Notification.Name("MPVPlayerEvent"),
                object: nil,
    ...
    @objc static func requiresSetup() -> Bool { return false }
}
