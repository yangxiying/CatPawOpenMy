...
- 构建 NodeMobile 运行时入口：cd nodejs && npm run build（产出 dist/nodejs-runtime.js）

先看哪些文件（优先级）
- nodejs/src/index.js — 蜘蛛服务入口，导出 start(config)/stop()（嵌入到移动端）。
- nodejs/src/main.js — NodeMobile 运行时入口（builtinModules + catServerFactory + loadScript + rn-bridge）。
- nodejs/src/dev.js — 本地 launcher，设置 global catServerFactory / catDartServerPort 并在 dev 下调用 start()。
- nodejs/esbuild.js — 构建 dist/index.js + dist/nodejs-runtime.js 与 .md5（必须存在）。
- nodejs/package.json — 脚本为权威（dev/build/_build/build:config），不要绕过。
- CatPlayer/overlay/src/node/NodeService.tsx — NodeMobile 启动 + 远程源下载 + 直连 HTTP 请求。核心改造文件。
- CatPlayer/setup.sh — iOS 本地初始化与 CI 差异说明。管理 Frameworks 提取、polyfill 移除。

关键约束（改动前必须知道）
- 这是嵌入运行的 Node 服务：start(config) 会被宿主多次调用，不要假设单次进程生命周期。
- 开发端口 DEV_HTTP_PORT=3006；在 App 中运行时使用端口 0（由系统分配）。
- main.js 是 NodeMobile 运行时入口，仅含基础设施（builtinModules / catServerFactory / loadScript / rn-bridge），不内嵌 spider。
- 远程源通过 rn-bridge 的 `action:'run'` 加载到 NodeMobile，不再使用 WebView polyfill。
- NodeService 的 API 请求通过 `fetch('http://127.0.0.1:{port}')` 直连 HTTP，不走 postMessage 桥。
- NodeMobile.framework 从 nodejs-mobile-react-native 的 xcframework 提取（~52MB），setup.sh 中复制到 app/Frameworks/。
...
- —minimal 模式下不安装 nodejs-mobile-react-native、不提取 NodeMobile.framework。
- 使用 `nodejs-mobile-react-native` 的 rn-bridge 实现 RN ↔ Node.js 双向通信，替代 MiraPlay 的 myaddon `_linkedBinding`。
